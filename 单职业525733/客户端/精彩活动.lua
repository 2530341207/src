...
local showtable = {
    {
        ...
        "激情派对		经验暴涨",
        "福利活动",
        "活动时间地图出现光圈，占领光圈范围内即可获得奖励位置不同获得奖励不同，最高每秒可获得400万经验+10元宝",
        "新区35分钟后举行（每天下午18:00举行）",
        "获得大量经验+元宝奖励",
        "全服所有玩家均可参与",
        "进入激情派对"
    },
    {
        ...
        "天赐宝箱		全面竞速",
        "时间竞速",
        "活动地图内禁止使用随机传送从入口跑到宝箱指定位置即可打开宝箱，没抽完一次都将送你回到入口处，宝箱100个抽完活动结束",
        "新区60分钟后举行（每天下午15举行）",
        "随机获得本服所有物品",
        "等级达到65级以上玩家都可以参与",
        "进入天赐宝箱"
...
function activity.submit(this)
    ...
    elseif num == 2 then
        UI:Lua_SubmitForm("天赐宝箱", "receive", tostring(num))
    elseif num == 3 then
        UI:Lua_SubmitForm("精彩活动", "receive", tostring(num))
    elseif num == 4 then
        UI:Lua_SubmitForm("精彩活动", "receive", tostring(num))
