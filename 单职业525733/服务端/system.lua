...
function on_system_start()
    --服务器第一次启动执行
    if not lualib:HasTimer("0", 666138) then
        lualib:AddTimer("0", 666138, 1800000, -1, "BOSS首杀:summaryData")
    else
        lualib:DisableTimer("0", 666138)
        lualib:AddTimer("0", 666138, 1800000, -1, "BOSS首杀:summaryData")
    end
    ...
    lualib:AddTrigger("0",  319, "on_attack_ex")  --受伤触发
    ...
    lualib:DelayCall("0",1,"精彩活动:activityTimer","")
    lualib:AddTimer("0",20200515, 1000, -1, "精彩活动:activityTimer")
    lualib:AddTimer("0",202005192, 1000, -1, "天赐宝箱:treasureChestTimer")
    lualib:AddTimer("0",202005251, 1000, -1, "行会争霸表单:active")
    lualib:AddTimer("0",202005252, 1000, -1, "全英争霸:active")
    lualib:AddTimer("0",202005253, 1000, -1, "全民押镖:active")
    lualib:AddTimer("0",202005254, 1000, -1, "垂钓休闲:active")
    return
end
...
function on_attack_ex(beAttacker, attacker, hp, skillKey)
    local bg = lualib:GetInt(attacker, '倍攻')
    if bg ~= 0 then
        hp = math.floor((hp * bg) / 100 )
    end
    return hp
end
...
local skill_tb = {
    "剑术心法4级","攻心斩4级","风刺剑法4级","月弧弯刀4级","野蛮冲锋4级","烈焰斩4级","裂地斩4级","龙咆哮4级"
}
...
function on_first_in_game(player)
    ...
    local item_tb = {"武器1","衣服1","头盔1","项链1","护腕1","护腕1","戒指1","戒指1","腰带1","鞋子1"}
    for i=1,#item_tb do
        lualib:AddItem(player,item_tb[i],1,true,"给物品：第一次进游戏",player)
    end
    for i=1,#item_tb do
        lualib:ApplyEquipEx(player,item_tb[i])
    end
    for i=1,#skill_tb do
        lualib:AddSkill(player,skill_tb[i])
    end

    lualib:AddItem(player,"QQ礼包",1,true,"给物品：第一次进游戏",player)
    lualib:AddItem(player,"微信礼包",1,true,"给物品：第一次进游戏",player)

    if lualib:GetAllTime() <= lualib:Str2Time(testtime)  then
        lualib:Player_MapMoveXY(player, "新西域", 100, 167, 1)
    ...
    elseif lualib:GetAllTime() <= lualib:Str2Time(opentime)  then
        ...
        lualib:Player_MapMoveXY(player, "新西域", 100, 167, 1)
        local s = "尊敬的玩家：您好！《".. lualib:GetZoneName() .."》服务器刚开一秒，请玩家们快去体验游戏的乐趣吧！"
        lualib:SysMsg_SendBottomColor(player, s, 7, 2)
    end

    local playername = lualib:KeyName(player)
    lualib:SysMsg_SendBroadcastMsg("玩家【"..playername.."】加入游戏,为《久玩神途》添加一份力量。", "[系统提示]")
...
function on_login(player)
    ...
    -- vip
    local vipLevel = lualib:GetInt(player, "vipLevel")
    if vipLevel > 0 then
        for i=1, 6 do
            if lualib:HasBuff(player, "VIP"..tostring(i).."级") then
                lualib:DelBuff(player, "VIP"..tostring(i).."级")
            end
        end
        lualib:AddBuff(player, "VIP"..tostring(vipLevel).."级", 0)
    end
...
function on_post_monster_die(monster, Killer)
    ...
    if lualib:Monster_IsMonster(Killer) then
        local owner = lualib:Monster_GetMaster(Killer)
        owner = lualib:Name2Guid(owner)
        if owner ~= "" then
            if ShareConstant.firstDropEquipIndexTable[monster_keyName] ~= nil then
                -- 设置怪物首杀信息
                local isFirstDrop = lualib:GetDBStr(monster_keyName.."是否首杀")
                if isFirstDrop == "" then
                    lualib:SetDBStr(monster_keyName.."是否首杀", "已击杀")
                    lualib:SetDBStr(monster_keyName.."首杀玩家", lualib:Name(owner))
                    lualib:SysMsg_SendBroadcastMsg("恭喜玩家#COLORCOLOR_SKYBLUE#"..lualib:Name(owner).."#COLOREND#拿到#COLORCOLOR_GREENG#"..lualib:Name(monster).."#COLOREND#首杀，让我们一起恭喜他！！！", "系统")
                    lualib:MailToPlayer("BOSS首杀", owner, "恭喜您成为全服第一位首杀"..lualib:Name(monster).."的人，祝贺！！！这是我们为您准备的奖励~希望再接再厉！", 0, tonumber(ShareConstant.firstDropEquipIndexTable[monster_keyName]), {"金币1", 1, 1})
                end
            end
        end
    else
        if ShareConstant.firstDropEquipIndexTable[monster_keyName] ~= nil then
            ...
            if isFirstDrop == "" then
                lualib:SetDBStr(monster_keyName.."是否首杀", "已击杀")
                lualib:SetDBStr(monster_keyName.."首杀玩家", lualib:Name(Killer))
                lualib:SysMsg_SendBroadcastMsg("恭喜玩家#COLORCOLOR_SKYBLUE#"..lualib:Name(Killer).."#COLOREND#拿到#COLORCOLOR_GREENG#"..lualib:Name(monster).."#COLOREND#首杀，让我们一起恭喜他！！！", "系统")
                lualib:MailToPlayer("BOSS首杀", Killer, "恭喜您成为全服第一位首杀"..lualib:Name(monster).."的人，祝贺！！！这是我们为您准备的奖励~希望再接再厉！", 0, tonumber(ShareConstant.firstDropEquipIndexTable[monster_keyName]), {"金币1", 1, 1})
            end
        end
    end
